...
import { ShieldCheck, Search, Loader2, ArrowRight, X } from 'lucide-react';
...
export const AutoDetectAnimation: React.FC<AutoDetectAnimationProps> = ({
  ...
  useEffect(() => {
    // Simulate an analyzing scan for 1.2 seconds, then transition to detected
  ...
  // Set workspace theme parameters
  const workspaceColors: Record<string, { bg: string; text: string; border: string; from: string; to: string; label: string }> = {
    ...
    office: {
      ...
      to: 'to-teal-600',
      label: 'Office & Text Suite'
    }
  };

  const wsMeta = workspaceColors[targetWorkspace] || {
    bg: 'bg-primary/10',
    text: 'text-primary',
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {step === 'analyzing' ? (
            ...
            >
              {/* Spinning/pulsing magnifying glass icon */}
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground max-w-[280px] truncate mx-auto">
                  Verifying header signatures of <span className="font-semibold">{fileName}</span>
                </p>
              </div>
              ...
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
                Inspecting structural metadata...
              </p>
            </motion.div>
          ) : (
            <motion.div
              ...
              className="space-y-6 py-4 flex flex-col"
            >
              ...
              <div className="p-4 bg-muted/50 border border-border rounded-xl space-y-2 text-sm leading-relaxed">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-bold border ${wsMeta.bg} ${wsMeta.text} ${wsMeta.border}`}>
                    Direct Mode
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <button
                  onClick={onConfirm}
                  className={`flex-1 py-3 px-4 bg-gradient-to-r ${wsMeta.from} ${wsMeta.to} text-white font-extrabold rounded-xl shadow-premium hover:shadow-premium-lg hover:scale-[1.02] transition-all flex items-center justify-center space-x-2`}
                >
                ...
                <button
                  onClick={onCancel}
                  className="py-3 px-4 bg-card hover:bg-muted border border-border text-muted-foreground hover:text-foreground font-bold rounded-xl transition-all"
                >
                  Cancel
                </button>
              </div>
