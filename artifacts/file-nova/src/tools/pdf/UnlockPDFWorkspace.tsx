...
export const UnlockPDFWorkspace: React.FC = () => {
  ...
  const configPanel = (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <label className="text-xs font-black uppercase tracking-wider text-muted-foreground">PDF Password (if encrypted)</label>
      ...
      <div className="bg-card border border-border/50 rounded-2xl p-4 flex gap-2.5 text-xs text-muted-foreground leading-relaxed font-medium">
        ...
        <div className="space-y-1">
          <p className="font-bold text-foreground/80">Important Decryption Notice</p>
  ...
  const previewPanel = (
    <div className="text-center py-6">
      <LockKeyhole className="h-10 w-10 text-primary mx-auto mb-2 animate-pulse" />
      <p className="text-xs font-bold text-foreground/90">Locked PDF Document</p>
      <p className="text-[10px] text-muted-foreground mt-1">Upload encrypted files. Previews are hidden until decrypted.</p>
    </div>
  );
