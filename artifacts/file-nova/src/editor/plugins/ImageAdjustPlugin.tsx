...
const ImageAdjustSection: React.FC<SectionProps> = ({ config, onConfigChange, mode, disabled }) => (
  <div className="space-y-3">
    <BentoCard size="sm" hover={false}>
      <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400 dark:text-slate-500 mb-2 flex items-center gap-1.5">
    ...
    <BentoCard size="sm" hover={false}>
      <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400 dark:text-slate-500 mb-2">Transform</p>
      <div className="grid grid-cols-3 gap-2">
        <button type="button" onClick={() => onConfigChange("rotation", (config.rotation || 0) - 90)}
          className="flex items-center justify-center gap-1 rounded-xl border border-slate-200 dark:border-white/10 bg-white/80 dark:bg-slate-950 px-3 py-2 text-xs font-bold text-slate-500 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-900 hover:border-emerald-500/30 transition cursor-pointer" disabled={disabled}>
    ...
    <BentoCard size="sm" hover={false}>
      <SettingRow label="Filter">
        <select value={config.filterPreset || "none"} onChange={(e) => onConfigChange("filterPreset", e.target.value)}
          className="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-3 py-2 text-xs text-slate-600 dark:text-slate-300 outline-none focus:border-emerald-500" disabled={disabled}>
    ...
    {mode === "advanced" && (
      <label className="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-white/10 bg-white/80 dark:bg-slate-950/60 px-4 py-2.5 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition">
        <input type="checkbox" checked={!!config.sharpness} onChange={(e) => onConfigChange("sharpness", e.target.checked)}
          className="h-4 w-4 rounded border-slate-300 dark:border-slate-700 text-emerald-500 focus:ring-emerald-500/30 cursor-pointer" disabled={disabled} />
        <span className="text-xs font-bold text-slate-600 dark:text-slate-300">Apply Sharpness Enhancement</span>
      </label>
    )}
  </div>
);
  ...
  <div className="space-y-3">
      ...
      <SettingRow label="Dimensions">
        <div className="grid grid-cols-2 gap-2">
          <input type="number" value={config.resizeWidth || 800} onChange={(e) => onConfigChange("resizeWidth", Number(e.target.value))}
            ...
            className="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-3 py-2 text-xs text-slate-800 dark:text-white outline-none focus:border-emerald-500" disabled={disabled} />
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <button key={p.label} type="button" onClick={() => { onConfigChange("resizeWidth", p.w); onConfigChange("resizeHeight", p.h); }}
              className="rounded-xl border border-slate-200 dark:border-white/10 bg-white/80 dark:bg-slate-950 px-3 py-2 text-[11px] font-bold text-slate-500 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-900 hover:border-emerald-500/30 transition cursor-pointer" disabled={disabled}>
              {p.label} <span className="text-[9px] text-slate-400">({p.w}×{p.h})</span>
            </button>
          ))}
        </div>
      </SettingRow>
    ...
    <label className="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-white/10 bg-white/80 dark:bg-slate-950/60 px-4 py-2.5 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition">
      <input type="checkbox" checked={!!config.maintainAspect} onChange={(e) => onConfigChange("maintainAspect", e.target.checked)}
        className="h-4 w-4 rounded border-slate-300 dark:border-slate-700 text-emerald-500 focus:ring-emerald-500/30 cursor-pointer" disabled={disabled} />
      <span className="text-xs font-bold text-slate-600 dark:text-slate-300">Maintain Aspect Ratio</span>
    </label>
  </div>
);
