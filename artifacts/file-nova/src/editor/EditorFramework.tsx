...
const EditorFramework: React.FC<EditorFrameworkProps> = ({ file, fileType, plugin, onClose, onDone, totalPages: _totalPages, loading = false }) => {
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-50 flex flex-col bg-card dark:bg-background text-foreground dark:text-foreground font-sans overflow-hidden">
      {/* ── Top Toolbar ── */}
      <header className="flex items-center justify-between gap-3 border-b border-border dark:border-border bg-card/80 dark:bg-card/80 backdrop-blur-xl px-4 py-2.5 shrink-0 z-20">
        <div className="flex items-center gap-3 min-w-0">
          <button onClick={onClose} aria-label="Close editor"
            className="h-8 w-8 flex items-center justify-center rounded-xl border border-border dark:border-muted bg-muted/80 dark:bg-muted hover:bg-muted dark:hover:bg-muted text-muted-foreground dark:text-foreground/90 transition cursor-pointer">
            <X className="h-4 w-4" />
          </button>
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground/80 dark:text-muted-foreground">{plugin.name}</p>
            <p className="text-sm font-bold text-foreground dark:text-foreground truncate max-w-[200px]">{file?.name || "Untitled"}</p>
          </div>
        </div>
          ...
          {plugin.presets && plugin.presets.length > 0 && (
            <div className="relative">
              <button onClick={() => setShowPresets(!showPresets)}
                className="h-8 px-3 flex items-center gap-1.5 rounded-xl border border-border dark:border-muted bg-muted/80 dark:bg-muted hover:bg-muted dark:hover:bg-muted text-muted-foreground dark:text-foreground/90 text-xs font-bold transition cursor-pointer">
                <Wand2 className="h-3.5 w-3.5" /> Presets
              </button>
              <AnimatePresence>
                {showPresets && (
                  <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                    className="absolute top-full right-0 mt-1 w-56 rounded-2xl border border-border dark:border-muted bg-card dark:bg-card shadow-2xl shadow-black/10 overflow-hidden z-30">
                    <div className="p-2 space-y-1">
                      {plugin.presets.map((p) => (
                        <button key={p.id} type="button" onClick={() => applyPreset(p.config)}
                          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-muted/80 dark:hover:bg-muted text-left transition cursor-pointer">
                          <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-muted/80 dark:bg-muted text-muted-foreground dark:text-foreground/90">{p.icon}</span>
                          <div>
                            <p className="text-xs font-bold text-foreground/90 dark:text-foreground/90">{p.label}</p>
                            {p.description && <p className="text-[9px] text-muted-foreground/80 dark:text-muted-foreground">{p.description}</p>}
                          </div>
                        </button>
                      ))}
                    </div>
          ...
          <button onClick={() => setMode(mode === "beginner" ? "advanced" : "beginner")}
            className={[
              ...
              mode === "advanced"
                ...
                : "border-border dark:border-muted bg-muted/80 dark:bg-muted text-muted-foreground dark:text-foreground/90"
            ].join(" ")}>
            {mode === "advanced" ? "Advanced" : "Beginner"}
          </button>
          <div className="w-px h-6 bg-muted dark:bg-muted/80 mx-0.5" />
          <button onClick={() => setZoom((z) => Math.max(0.5, z - 0.1))} aria-label="Zoom out"
            className="h-8 w-8 flex items-center justify-center rounded-xl border border-border dark:border-muted bg-muted/80 dark:bg-muted hover:bg-muted dark:hover:bg-muted text-muted-foreground dark:text-foreground/90 transition cursor-pointer">
            <ZoomOut className="h-3.5 w-3.5" />
          </button>
          <span className="text-xs font-bold text-muted-foreground/80 dark:text-muted-foreground w-10 text-center">{Math.round(zoom * 100)}%</span>
      ...
      <button onClick={() => setMobileSheetOpen(!mobileSheetOpen)}
        className="lg:hidden flex items-center justify-center gap-2 py-2.5 border-b border-border dark:border-border bg-muted/50 dark:bg-card/60 text-xs font-bold text-muted-foreground/80 dark:text-muted-foreground hover:text-foreground/80 dark:hover:text-foreground transition cursor-pointer">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <aside className="hidden lg:flex w-[280px] shrink-0 flex-col border-r border-border dark:border-border bg-muted/50/50 dark:bg-card/40 overflow-y-auto">
          <div className="p-3 space-y-2.5">
            {loading ? (
              <EditorSkeleton sectionCount={visibleSections.length || 3} />
            ) : visibleSections.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground/80 dark:text-muted-foreground">
              ...
              return (
                <motion.div
                  ...
                  className="overflow-hidden rounded-2xl border border-border dark:border-border bg-card/80 dark:bg-background/60 backdrop-blur-sm shadow-sm"
                >
                  <button type="button" onClick={() => toggleSection(section.id)}
                    className="flex w-full items-center justify-between px-4 py-3 text-left transition hover:bg-muted/80 dark:hover:bg-muted/40 cursor-pointer">
                    <div className="flex items-center gap-2.5">
                      <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-muted/80 dark:bg-muted text-muted-foreground dark:text-foreground/90">{section.icon}</span>
                      <div>
                        <p className="text-xs font-bold text-foreground/90 dark:text-foreground/90">{section.label}</p>
                        {section.description && <p className="text-[9px] text-muted-foreground/80 dark:text-muted-foreground">{section.description}</p>}
                      </div>
                    </div>
                    <motion.span animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.2 }} className="text-muted-foreground/80 dark:text-muted-foreground">
        ...
        {/* ── Center Live Preview ── */}
        <main className="flex-1 flex flex-col min-w-0 bg-muted/80/50 dark:bg-background">
          <div className="flex-1 flex items-center justify-center overflow-auto p-4">
            {plugin.previewComponent ? (
              ...
              <div className="flex flex-col items-center gap-4 w-full">
                ...
                <div className="flex items-center gap-3 bg-card/80 dark:bg-card/80 backdrop-blur-lg px-4 py-2 rounded-2xl border border-border dark:border-border shadow-sm">
                  <button onClick={() => setPdfPage((p) => Math.max(1, p - 1))} disabled={pdfPage === 1}
                    className="px-3 py-1.5 rounded-xl bg-muted/80 dark:bg-muted hover:bg-muted dark:hover:bg-muted text-xs font-bold text-muted-foreground dark:text-foreground/90 disabled:opacity-40 transition cursor-pointer border border-border dark:border-muted">← Prev</button>
                  <span className="text-xs font-bold text-muted-foreground dark:text-muted-foreground/80">Page {pdfPage} of {pdfDoc?.numPages || "—"}</span>
                  <button onClick={() => setPdfPage((p) => Math.min(pdfDoc?.numPages || 1, p + 1))} disabled={pdfPage === (pdfDoc?.numPages || 1)}
                    className="px-3 py-1.5 rounded-xl bg-muted/80 dark:bg-muted hover:bg-muted dark:hover:bg-muted text-xs font-bold text-muted-foreground dark:text-foreground/90 disabled:opacity-40 transition cursor-pointer border border-border dark:border-muted">Next →</button>
                  <div className="w-px h-5 bg-muted dark:bg-muted/80" />
                  <select value={pdfZoom} onChange={(e) => setPdfZoom(Number(e.target.value))}
                    className="rounded-xl border border-border dark:border-muted bg-muted/80 dark:bg-muted text-foreground/80 dark:text-foreground px-2 py-1.5 text-xs font-bold outline-none">
              ...
              <div className="flex flex-col items-center justify-center text-muted-foreground/80 dark:text-muted-foreground">
                <div className="h-16 w-16 rounded-2xl bg-muted/80 dark:bg-muted flex items-center justify-center mb-3">
                  <FileText className="h-8 w-8" />
                </div>
                <p className="text-sm font-bold text-muted-foreground dark:text-muted-foreground/80">No preview available</p>
                <p className="text-xs mt-1 text-muted-foreground/80 dark:text-muted-foreground">Load a file to see the preview</p>
              </div>
            )}
          </div>
        </main>
        ...
        {RightPanel && showRightPanel && (
          <aside className="hidden lg:flex w-[240px] shrink-0 flex-col border-l border-border dark:border-border bg-muted/50/50 dark:bg-card/40 overflow-y-auto p-3">
      ...
      {/* ── Bottom Status Bar ── */}
      <footer className="flex items-center justify-between border-t border-border dark:border-border bg-card/80 dark:bg-card/80 backdrop-blur-lg px-4 py-2 shrink-0">
        <div className="flex items-center gap-2">
          <span className={`h-1.5 w-1.5 rounded-full ${busy ? "bg-amber-400 animate-pulse" : "bg-emerald-500"}`} />
          <span className="text-[11px] font-medium text-muted-foreground dark:text-muted-foreground/80">{busy ? "Processing..." : statusMessage}</span>
        </div>
        <div className="flex items-center gap-3 text-[10px] text-muted-foreground/80 dark:text-muted-foreground">
          <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-slate-300 dark:bg-muted/60" /> {file?.type || "No file"}</span>
          <span className="w-px h-3 bg-muted dark:bg-muted/80" />
          <span>{annotations.length > 0 ? `${annotations.length} annotation${annotations.length > 1 ? "s" : ""}` : "No annotations"}</span>
          <span className="w-px h-3 bg-muted dark:bg-muted/80" />
          <span>{Math.round(zoom * 100)}%</span>
          {mode === "advanced" && <span className="w-px h-3 bg-muted dark:bg-muted/80" />}
      ...
      <AnimatePresence>
        {mobileSheetOpen && (
          <motion.div
            ...
            className="lg:hidden fixed bottom-0 left-0 right-0 z-50 max-h-[65vh] bg-card dark:bg-card border-t border-border dark:border-border rounded-t-2xl overflow-y-auto shadow-2xl"
          >
            <div className="sticky top-0 bg-card dark:bg-card border-b border-border dark:border-border px-4 py-2.5 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-muted-foreground dark:text-muted-foreground/80">{plugin.name} Controls</span>
              </div>
              <button onClick={() => setMobileSheetOpen(false)} className="h-7 w-7 flex items-center justify-center rounded-lg hover:bg-muted/80 dark:hover:bg-muted text-muted-foreground/80 dark:text-muted-foreground transition cursor-pointer">
                <ChevronDown className="h-4 w-4" />
              </button>
            </div>
                ...
                return (
                  <BentoCard key={section.id} size="sm">
                    <div className="flex items-center gap-2 mb-2.5">
                      <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-muted/80 dark:bg-muted text-muted-foreground dark:text-foreground/90">{section.icon}</span>
                      <p className="text-xs font-bold text-foreground/90 dark:text-foreground/90">{section.label}</p>
                      {section.advanced && <span className="text-[8px] px-1 py-0.5 rounded bg-muted/80 dark:bg-muted text-muted-foreground/80 dark:text-muted-foreground font-bold">ADV</span>}
                    </div>
                    <SectionComponent {...sectionProps} />
                  </BentoCard>
                );
