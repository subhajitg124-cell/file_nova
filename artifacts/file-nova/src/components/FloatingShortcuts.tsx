  ...
  return (
    <div ref={containerRef} className="fixed bottom-6 right-6 z-50 font-sans flex items-end justify-end">
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.92 }}
        className={`relative z-50 flex items-center justify-center w-14 h-14 rounded-full text-white cursor-pointer shadow-premium transition-all duration-300 ${
          isOpen
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 w-full rounded-t-3xl border-t border-white/10 bg-slate-950/98 backdrop-blur-2xl p-6 z-45 sm:absolute sm:bottom-18 sm:right-0 sm:left-auto sm:w-[360px] sm:rounded-3xl sm:border sm:border-white/10 sm:p-5 sm:shadow-2xl overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between mb-4">
              ...
              <button 
                onClick={() => setIsOpen(false)}
                className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center bg-white/[0.02] hover:bg-white/5 active:scale-95 transition cursor-pointer sm:hidden"
              ...
              {actions.map((act) => (
                ...
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="w-9 h-9 rounded-xl bg-white/[0.03] border border-white/5 flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
                      {act.icon}
                    </div>
                    <ChevronRight className="w-3.5 h-3.5 text-white/30 group-hover:text-white/60 group-hover:translate-x-0.5 transition duration-200" />
                  </div>
                  <div>
                    <span className="block text-xs font-black text-white/95 leading-none">
                      {act.label}
                    </span>
                    <span className="block text-[9px] font-semibold text-white/50 truncate max-w-[125px] mt-1 leading-none">
                      {act.description}
                    </span>
                  </div>
                </button>
