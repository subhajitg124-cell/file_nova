...
import { 
  ...
  Camera, Wallet, BadgePercent, MessageCircle, PanelRightOpen, Star, Clock, Heart
} from "lucide-react";
    ...
    >
      ...
      <section className="pt-20 pb-16 px-4 relative z-10 text-center">
        <div className="max-w-3xl mx-auto">
          ...
          {/* Community-Funded Badge */}
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.4, delay: 0.25 }}
            className="mb-12 max-w-xl mx-auto p-4 rounded-2xl border border-rose-100 dark:border-rose-500/20 bg-rose-50/50 dark:bg-rose-950/20 backdrop-blur-md text-left flex items-start gap-3 shadow-card"
          >
            <div className="h-9 w-9 rounded-xl bg-rose-100/50 dark:bg-rose-500/10 flex items-center justify-center text-rose-600 dark:text-rose-400 shrink-0 border border-rose-200 dark:border-rose-500/20">
              <Heart className="h-4.5 w-4.5 fill-rose-500/20 dark:fill-rose-400/20 text-rose-500" />
            </div>
            <div className="space-y-0.5">
              <span className="block text-[11px] font-black text-foreground uppercase tracking-wider">
                {tText("100% Free & Community Funded")}
              </span>
              <p className="text-[10px] text-muted-foreground leading-relaxed">
                {tText("FileNova is now fully free with unlimited workspace actions and up to 100MB file uploads for all. If you love our service, please consider ")}
                <Link href="/pricing" className="text-rose-500 hover:underline font-extrabold">
                  {tText("supporting us")}
                </Link>
                {tText(" to help fund server operations!")}
              </p>
            </div>
          </motion.div>
      ...
      <section className="py-16 bg-background border-t border-border relative z-10">
        <div className="max-w-5xl mx-auto px-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-8">
            <div>
              <p className="text-[10px] font-black uppercase tracking-wider text-brand-primary">Recommended</p>
              <h2 className="mt-1 text-xl md:text-2xl font-black text-foreground font-display">
                Useful services for document desks
              </h2>
              <p className="mt-2 text-xs text-muted-foreground max-w-lg">
                Curated partner tools for hosting, design, backup, and operator workflows. Sponsored placements are always labeled.
              </p>
            </div>
            <Link href="/partners" className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2 text-xs font-black text-foreground transition hover:bg-muted">
              View all partners
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {partnerRecommendations.map((partner) => (
              <a
                ...
                className="group rounded-2xl border border-border bg-card p-5 transition hover:border-brand-primary/40 hover:shadow-[var(--fn-shadow-card)]"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-black text-foreground group-hover:text-brand-primary transition-colors">{partner.name}</p>
                    <p className="mt-1 text-xs font-semibold text-muted-foreground">{partner.tagline}</p>
                  </div>
                ...
                <p className="mt-4 text-xs leading-relaxed text-muted-foreground line-clamp-3">{partner.description}</p>
                <span className="mt-5 inline-flex items-center gap-1 text-xs font-black text-brand-primary">
                  Visit service <ArrowUpRight className="h-3.5 w-3.5" />
                </span>
              </a>
            ))}
          </div>
