...
export const ProgressTracker: React.FC = () => {
  ...
  const getStepDetails = (prog: number) => {
    if (prog < 25) {
      return { label: 'Verifying integrity', desc: 'Analyzing headers & magic bytes validation...', icon: HardDriveUpload, color: 'text-blue-500' };
    }
    if (prog >= 25 && prog < 55) {
      return { label: 'Initializing core', desc: 'Parsing parameters and importing plugin modules...', icon: Settings, color: 'text-violet-500' };
    }
    if (prog >= 55 && prog < 90) {
      return {
    ...
    return { label: 'Wrapping up', desc: 'Rebuilding containers & generating temporary download links...', icon: FileCheck, color: 'text-emerald-500' };
  };
